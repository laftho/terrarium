...
RELASE_ID=$(jq -r '.id' <<< curl -u $username:$token -d "{ \"tag_name\": \"${tag}\", \"target_commitish\": \"master\", \"name\": \"${tag}\", \"body\": \"Terrarium release\" }" -H "Content-Type: application/json" -X POST https://api.github.com/repos/laftho/terrarium/releases)
